...
namespace MAAS_SFRThelper.Services
{
    ...
    public class OptimizationObjectiveCreator
    {
        ...
        public string CreateObjectives(
            ...
            OptimizationTemplate template,
            List<OARConstraint> oarConstraints = null,
            DoseValue.DoseUnit doseUnit = DoseValue.DoseUnit.Gy)
        {
            string result = "=== Creating SFRT Optimization Objectives ===\n";
            ...
            // Step 4: Add peak objectives (lattice/spheres)
            result += AddPeakObjectives(latticeStructure, template, doseUnit);

            // Step 5: Add valley objectives (if we have a valley structure)
            if (valleyStructure != null)
            {
                result += AddValleyObjectives(valleyStructure, template, doseUnit);
            }
            else
            {
                result += "\nSkipping valley objectives (no valley structure specified)";
            ...
            // Step 6: Add OAR objectives using the provided list
            result += AddOARObjectives(oarConstraints, doseUnit);
        ...
        /// Goal: Drive HIGH dose into these structures
        /// </summary>
        private string AddPeakObjectives(Structure latticeStructure, OptimizationTemplate template, DoseValue.DoseUnit doseUnit)
        {
            string result = "\n--- Adding Peak Objectives (Lattice) ---";
            string unitLabel = doseUnit == DoseValue.DoseUnit.cGy ? "cGy" : "Gy";
            ...
            try
            {
                ...
                _plan.OptimizationSetup.AddPointObjective(
                    latticeStructure,
                    OptimizationObjectiveOperator.Lower,
                    new DoseValue(template.PeakLowerDose, doseUnit),
                    template.PeakLowerVolume,
                    template.PeakLowerPriority);

                result += $"\n  Lower: {template.PeakLowerDose} {unitLabel} @ {template.PeakLowerVolume}% (Priority {template.PeakLowerPriority})";

                // Mean dose objective: Drive high average dose
                _plan.OptimizationSetup.AddMeanDoseObjective(
                    latticeStructure,
                    new DoseValue(template.PeakMeanDose, doseUnit),
                    template.PeakMeanPriority);

                result += $"\n  Mean:  {template.PeakMeanDose} {unitLabel} (Priority {template.PeakMeanPriority})";
            }
            catch (Exception ex)
            {
                result += $"\n  ERROR adding peak objectives: {ex.Message}";
        ...
        /// Goal: Keep dose LOW in these structures
        /// </summary>
        private string AddValleyObjectives(Structure valleyStructure, OptimizationTemplate template, DoseValue.DoseUnit doseUnit)
        {
            string result = "\n--- Adding Valley Objectives (Low Dose) ---";
            string unitLabel = doseUnit == DoseValue.DoseUnit.cGy ? "cGy" : "Gy";
            ...
            try
            {
                ...
                _plan.OptimizationSetup.AddPointObjective(
                    valleyStructure,
                    OptimizationObjectiveOperator.Upper,
                    new DoseValue(template.ValleyUpperDose, doseUnit),
                    template.ValleyUpperVolume,
                    template.ValleyUpperPriority);

                result += $"\n  Upper: {template.ValleyUpperDose} {unitLabel} @ {template.ValleyUpperVolume}% (Priority {template.ValleyUpperPriority})";

                // Mean dose objective: Keep average dose low
                _plan.OptimizationSetup.AddMeanDoseObjective(
                    valleyStructure,
                    new DoseValue(template.ValleyMeanDose, doseUnit),
                    template.ValleyMeanPriority);

                result += $"\n  Mean:  {template.ValleyMeanDose} {unitLabel} (Priority {template.ValleyMeanPriority})";
            }
            catch (Exception ex)
            {
                result += $"\n  ERROR adding valley objectives: {ex.Message}";
        ...
        /// Uses the provided constraint list (from ViewModel)
        /// </summary>
        private string AddOARObjectives(List<OARConstraint> oarConstraints, DoseValue.DoseUnit doseUnit)
        {
            string result = "\n--- Adding OAR Objectives ---";
            string unitLabel = doseUnit == DoseValue.DoseUnit.cGy ? "cGy" : "Gy";
            ...
            foreach (var constraint in oarConstraints)
            {
                ...
                try
                {
                    ...
                    _plan.OptimizationSetup.AddPointObjective(
                        oarStructure,
                        constraint.Operator,
                        new DoseValue(constraint.MaxDoseGy, doseUnit),
                        constraint.VolumePercent,
                        constraint.Priority);

                    result += $"\n  ✓ {oarStructure.Id}: {constraint.Operator} {constraint.MaxDoseGy} {unitLabel} @ {constraint.VolumePercent}% (Priority {constraint.Priority})";
                    addedCount++;
                }
                catch (Exception ex)
                {
