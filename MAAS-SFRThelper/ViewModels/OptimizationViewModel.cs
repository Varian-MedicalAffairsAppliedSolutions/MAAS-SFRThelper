...
//        #region Dose Unit Properties

//        public List<string> AvailableDoseUnits { get; } = new List<string> { "Gy", "cGy" };

//        private string _selectedDoseUnit = "Gy";
...
//                RaisePropertyChanged(nameof(DoseColumnHeader));
//                UpdateDoseUnitMismatchWarning();
//            }
//        }

//        private string _detectedDoseUnit;
//        public string DetectedDoseUnit
//        {
//            get { return _detectedDoseUnit; }
//            set { SetProperty(ref _detectedDoseUnit, value); }
//        }

//        private string _doseUnitMismatchWarning;
//        public string DoseUnitMismatchWarning
//        {
//            get { return _doseUnitMismatchWarning; }
//            set { SetProperty(ref _doseUnitMismatchWarning, value); }
//        }

//        private bool _showDoseUnitMismatch;
//        public bool ShowDoseUnitMismatch
//        {
//            get { return _showDoseUnitMismatch; }
//            set { SetProperty(ref _showDoseUnitMismatch, value); }
//        }

//        public string DoseColumnHeader => $"Dose ({SelectedDoseUnit})";

//        private void UpdateDoseUnitMismatchWarning()
//        {
//            if (!string.IsNullOrEmpty(DetectedDoseUnit) && DetectedDoseUnit != SelectedDoseUnit)
//            {
//                DoseUnitMismatchWarning = $"⚠ Your Eclipse system reports {DetectedDoseUnit}. You've selected {SelectedDoseUnit}.";
//                ShowDoseUnitMismatch = true;
//            }
//            else
//            {
//                DoseUnitMismatchWarning = "";
//                ShowDoseUnitMismatch = false;
//            }
//        }
...
//                try
//                {
//                    // Detect system dose unit and set as default
//                    string detectedUnit = "Gy";
//                    try
//                    {
//                        var externalPlan = sc.PlanSetup as ExternalPlanSetup;
//                        if (externalPlan?.TotalDose != null && externalPlan.TotalDose.Unit == DoseValue.DoseUnit.cGy)
//                        {
//                            detectedUnit = "cGy";
//                        }
//                        Output += $"\nDetected system dose unit: {detectedUnit}";
//                    }
//                    catch
//                    {
//                        Output += "\n⚠ Could not detect system dose unit. Defaulting to Gy. Please verify your selection.";
//                    }
...
//                    dispatcher.BeginInvoke(new Action(() =>
//                    {
//                        // Set detected unit and default selection
//                        DetectedDoseUnit = detectedUnit;
//                        SelectedDoseUnit = detectedUnit;
...
//                        Output += "\n\nFill in dose and priority values, then click 'Create Objectives' to apply to Eclipse.";
...
//            // Validate: check for zero dose values
//            var zeroDoseObjectives = includedObjectives.Where(o => o.Dose <= 0).ToList();
//            if (zeroDoseObjectives.Any())
//            {
//                var names = string.Join(", ", zeroDoseObjectives.Select(o => o.StructureName));
//                Output += $"\n⚠ WARNING: The following included objectives have no dose value set: {names}";
//                Output += "\nPlease enter dose values before creating objectives.";
//                return;
//            }

//            // Validate: check for zero priority values
//            var zeroPriorityObjectives = includedObjectives.Where(o => o.Priority <= 0).ToList();
//            if (zeroPriorityObjectives.Any())
//            {
//                var names = string.Join(", ", zeroPriorityObjectives.Select(o => o.StructureName));
//                Output += $"\n⚠ WARNING: The following included objectives have no priority set: {names}";
//                Output += "\nPlease enter priority values before creating objectives.";
//                return;
//            }

//            // Mismatch warning
//            string selectedUnit = SelectedDoseUnit;
//            if (!string.IsNullOrEmpty(DetectedDoseUnit) && DetectedDoseUnit != selectedUnit)
//            {
//                var result = MessageBox.Show(
//                    $"Your Eclipse system reports {DetectedDoseUnit} but you've selected {selectedUnit}.\n\n" +
//                    $"Objectives will be created with dose values in {selectedUnit}.\n\nContinue?",
//                    "Dose Unit Mismatch",
...
//                {
//                    Output += "\nObjective creation cancelled by user (dose unit mismatch).";
//                    return;
//                }
//            }

//            DoseValue.DoseUnit doseUnit = selectedUnit == "cGy"
//                ? DoseValue.DoseUnit.cGy
//                : DoseValue.DoseUnit.Gy;
...
//                                    structure,
//                                    objective.Operator,
//                                    new DoseValue(objective.Dose, doseUnit),
//                                    objective.Volume,
//                                    objective.Priority);

//                                Output += $"\n  ✓ {structure.Id}: {objective.Operator} {objective.Dose} {selectedUnit} @ {objective.Volume}% (Priority {objective.Priority})";
//                            }
...
//                                Output += $"\n  ✓ {structure.Id}: Mean {objective.Dose} {selectedUnit} (Priority {objective.Priority})";
//                            }
...
namespace MAAS_SFRThelper.ViewModels
{
    public class OptimizationViewModel : BindableBase
    {
        ...
        private void OnCalculateDose()
        {
            ...
            _esapiWorker.Run(sc =>
            {
                ...
                try
                {
                    ...
                    try
                    {
                        DisplayDVHSummary(externalPlan, sc.StructureSet);
                    }
                    catch (Exception ex)
                    {
                        try
        ...
        private void DisplayDVHSummary(ExternalPlanSetup plan, StructureSet structureSet)
        {
            Output += "\n\n=== DVH Summary ===";
            ...
            try
            {
                ...
                // Report every structure that has an optimization objective. These are
                // exactly the structures the plan was optimized against (peak/lattice,
                // valley, and OARs), so no name matching or dropdown lookups are needed.
                // The automatic normal-tissue objective has no StructureId and is skipped.
                foreach (var objective in plan.OptimizationSetup.Objectives)
                {
                    ...
                    if (!structuresToCheck.Any(s => s.Equals(structureId, StringComparison.OrdinalIgnoreCase)))
                        structuresToCheck.Add(structureId);
                }

                if (structuresToCheck.Count == 0)
                    Output += "\n  (No optimization objectives found on this plan.)";
